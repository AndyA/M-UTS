const genfun = require("generate-function");

const reSymbol = `(?:(?:[.:]?[a-z_]\\w*)(?:\.[a-z_]\\w*)*|-+|\\++)`;
const reHex = `(?:[$&]|0x)[0-9a-f]+`;
const reOct = `0o?[0-7]+`;
const reBin = `(?:%|0b)[0-1]+`;
// const reDirective = `(?:[!+]?[a-z]\\w*)`;

function put(obj, value, ...path) {
  const [next, ...p] = [...path];
  if (p.length) put((obj[next] = obj[next] || {}), value, p);
  else obj[next] = value;
}

const reTokenise = new RegExp(
  `(` +
    [
      `\\\\.`,
      `\\\${`,
      reHex,
      reOct,
      reBin,
      reSymbol,
      `\\&\\&`,
      `\\|\\|`,
      `<=`,
      `>=`,
      `===`,
      `==`,
      `!==`,
      `!=`,
      `[~!-+*/%<>?:."'\`(){}\\[\\]\\|\\&]`,
      `\\s+`
    ].join("|") +
    ")",
  "i"
);

const exTokenise = expr =>
  expr.split(reTokenise).filter(s => s !== undefined && !/^\s*$/.test(s));

const parenPair = { "(": ")", "{": "}", "[": "]" };

const exParse = expr => {
  const toks = exTokenise(expr);

  const parse = () => {
    const node = [];
    const stash = tag => node.push({ tag, tokens: parse() });

    while (toks.length) {
      const peek = toks[0];

      if (")]}".includes(peek)) break;

      const closer = parenPair[peek];
      if (closer) {
        stash(toks.shift());

        const next = toks.shift();
        if (next === closer) continue;
        throw new Error(`Missing "${closer}" (got "${next}")`);
      }

      if ("'\"`".includes(peek)) {
        let str = [];

        const flush = () => {
          node.push(JSON.stringify(str.join("")));
          str = [];
        };

        toks.shift();

        while (true) {
          if (!toks.length) throw new Error(`Missing ${peek}`);
          const next = toks.shift();
          if (next === peek) break;

          // Handle nested
          if (peek === "`" && next === "${") {
            flush();
            node.push("+");
            stash("template");
            node.push("+");
            const closer = toks.shift();
            if (closer !== "}") throw new Error(`Missing "}", got "${closer}"`);
            continue;
          }

          if (/^\\/.test(next)) str.push(JSON.parse(`"${next}"`));
          else str.push(next);
        }
        flush();
        continue;
      }

      node.push(toks.shift());
    }

    return node;
  };

  return { tokens: parse() };
};

function compileExpression(expr) {
  const toks = exTokenise(expr);
  console.log({ toks });

  const expectToken = () => {
    if (!toks.length) throw new Error(`Unexpected end of input`);
    return toks[0];
  };

  const getToken = () => {
    if (!toks.length) throw new Error(`Unexpected end of input`);
    return toks.shift();
  };

  const checkToken = token => {
    const next = getToken();
    if (next !== token)
      throw new Error(`Missing "${token}", got "${next}" instead`);
  };

  const parseBase = b => n => parseInt(n, b);
  const numberFormats = [
    [/^(?:%|0b)([0-1]+)$/, parseBase(2)],
    [/^(?:[$&]|0x)([0-9a-f]+)$/i, parseBase(16)],
    [/^0o?([0-7]+)$/i, parseBase(8)]
  ];

  const exNumber = num => {
    for (const [re, cvt] of numberFormats) {
      const m = num.match(re);
      if (m) return cvt(m[1]);
    }

    return Number(num);
  };

  const exSymbol = sym => {
    if (/^[a-z_]\w*(?:\.[a-z_]\w*)*$/.test(sym)) return sym;

    const ms = sym.match(/^([.:]?)([a-z_]\w*)$/i);
    if (ms) {
      const [, ns, sym] = ms;
      const space = ns === "." ? "local" : "global";
      return `v.${space}[${JSON.stringify(sym)}]`;
    }

    const mr = sym.match(/^(-+|\++)$/);
    if (mr) return `f.resolveRelative(${JSON.stringify(mr[1])})`;
  };

  const exAtom = () => {
    const peek = expectToken();

    switch (peek) {
      case "(":
        getToken();
        const res = exTop();
        checkToken(")");
        return ["(", res, ")"].flat();
      case "-":
      case "~":
        return [getToken(), exAtom()].flat();
      case "+":
        getToken();
        return exAtom();
      case "<":
        getToken();
        return [`f.low(`, exTop(), `)`].flat();
      case ">":
        getToken();
        return [`f.high(`, exTop(), `)`].flat();
    }

    const sym = exSymbol(peek);
    if (sym) {
      getToken();
      return [sym].flat();
    }

    const num = exNumber(peek);
    if (!isNaN(num)) {
      getToken();
      return [num];
    }
  };

  const mkOpMap = ops =>
    Array.isArray(ops) ? Object.fromEntries(ops.map(op => [op, op])) : ops;

  const exPass = (exUp, ops) => {
    const opMap = mkOpMap(ops);
    let complex = false;

    return () => {
      const expr = exUp();
      while (toks.length) {
        const peek = expectToken();
        const op = opMap[peek];
        if (!op) break;
        getToken();
        expr.push(op, exUp());
        complex = true;
      }

      return (complex ? ["(", ...expr, ")"] : expr).flat();
    };
  };

  const exMult = exPass(exAtom, ["*", "/", "%"]);
  const exAdd = exPass(exMult, ["+", "-"]);

  const exTop = () => {
    return exAdd();
  };

  return exTop();
}

class MOS6502Assembler {
  constructor(syntax) {
    this.makeEngine(syntax);
  }

  makeEngine(syntax) {
    const ops = syntax
      .map(op => op && op.split(/\s+/))
      .map((op, opcode) => op && [...op, opcode])
      .filter(Boolean);

    const dict = {};
    for (const [mnem, addr, opcode] of ops) put(dict, opcode, mnem, addr);

    // console.log(dict);
  }
}

class MOS6502Build {
  constructor(assembler) {
    this.assembler = assembler;
    this.symtab = {};
  }
}

module.exports = { MOS6502Assembler, MOS6502Build, compileExpression, exParse };
