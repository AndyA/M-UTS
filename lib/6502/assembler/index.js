  ...
  compileLines(lb) {
    ...
    const parseMacroArgs = tl => {
      ...
      while (tl.hasMore()) {
        ...
        if (peek === "{") break;
        children.push(symbolRequired(tl));
        if (!tl.hasNext(",")) break;
        tl.getToken();
      }
      return children;
    ...
    const analyzer = tl => {
      const getThing = () => {
...

        {
          ...
          if (m) {
            tl.getToken();
            const tag = m[1] === "+" ? "macroCall" : "directive";
            return {
              tag,
...

        {
          ...
          if (sym) {
            tl.getToken();
            // assignment?
            if (tl.hasMore() && tl.expectNext() === "=") {
              tl.getToken();
              return { tag: "assign", chilren: [sym, exprRequired(tl)] };
            }
            return sym;
          }
        }
