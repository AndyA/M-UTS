...
function exprTranslator(tl) {
  ...
  const exprMacroSymbol = () => {
    if (!tl.hasMore()) return;
    const peek = tl.expectNext();
    if (peek === "~") {
      tl.getToken();
      return { tag: "ref", children: [exprRequiredSymbol()] };
    }
    return exprSymbol();
  };
  ...
  const exprAtom = () => {
    ...
    switch (peek) {
      ...
      case "-": {
        ...
        tl.getToken();
        const res = exprAtom();
        if (res) return peek === "+" ? res : { tag: "negate", children: [res] };
      ...
      case ">": {
        tl.getToken();
        const value = peek === "<" ? "loByte" : "hiByte";
        return { tag: "rt", value, children: [exprRequired()] };
      }
  ...
  const exprTernIf = () => {
    ...
    if (tl.hasMore()) {
      ...
      if (peek === "?") {
        ...
        if (tl.getToken() !== ":") throw new SyntaxError(`Missing ":"`);
  ...
  return { expr, exprRequired, exprSymbol, exprMacroSymbol };
}
...
const exprMacroSymbolOptional = tl => exprTranslator(tl).exprMacroSymbol();
