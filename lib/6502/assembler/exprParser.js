...
const parseSymbol = sym => {
  ...
  if (/^[a-z_]\w*(?:\.[a-z_]\w*)+$/i.test(sym))
    return { tag: "sym.js", value: sym };

  if (sym === "*" || sym.toLowerCase() === "org") return { tag: "sym.pc" };

  const ms = sym.match(/^([.:]?)([a-z_]\w*)$/i);
  if (ms) {
    const [, ns, sym] = ms;
    const tag = ns === "." ? "sym.local" : "sym.global";
    return { tag, value: sym };
  }

  const mr = sym.match(/^(-+|\++)$/);
  if (mr) return { tag: "sym.relative", value: mr[1] };
};

function exprTranslator(tl) {
  const requireMaker = (tl, ex) => () => {
    const res = ex();
    if (!res) {
  ...
  const exprRequireAtom = requireMaker(tl, exprAtom);
  ...
  const makePassthrough = (exprUp, ...ops) => {
    ...
    const exprRequireUp = requireMaker(tl, exprUp);
  ...
  const exprRequired = requireMaker(tl, expr);
