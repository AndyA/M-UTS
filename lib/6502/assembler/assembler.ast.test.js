...
function getTests(tests) {
  const enabled = tests.filter(t => !t.off);
  const solo = enabled.filter(t => t.only);
  return solo.length ? solo : enabled;
}

describe("instructions", () => {
  const TST = (source, ast) => ({ source, ast });
  TST.only = (source, ast) => ({ source, ast, only: true });
  TST.off = (source, ast) => ({ source, ast, off: true });
  ...
  const tests = [
    ...
    TST("?(symName) = ?(symName) + 1 + .a[msg_len - 1]", [
      TC("assign", [
        ...
        TC("add", [
          ...
          TC("index", [
            TV("sym", ".a"),
            TC("sub", [TV("sym", "msg_len"), TV("number", 1)])
          ])
        ])
      ])
    ]),
    ...
    TST("BPL +", [
      TVC("opcode", "BPL", [TVC("opArg", ["rel"], [TV("sym", "+")])])
    ]),
  ...
  for (const { source, ast } of getTests(tests)) {
