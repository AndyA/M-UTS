...
class MOS6502Core {
  ...
  get mc() {
    return (this._mc = this._mc || this.makeMachine(this.syntax));
  }
  ...
  makeMachine(syntax) {
    ...
    const adModes = {
      ...
      "(zpg),Y": () => wrd(fb()) + this.Y, // (zp), y
      "(zpg,X)": () => wrd(fb() + this.Y), // (zp, x)
    ...
    // Escape instruction handler
    (([op, ...tail], handler) =>
      (dispatch[op] = () => {
        if (tail.every((sig, i) => mem[wrap(this.PC + i)] === sig)) {
          this.PC = wrap(this.PC + tail.length);
          handler(fw());
        } else {
          throw new Error(`Illegal instruction: ${op.toString(16)}`);
        }
      }))(escape, () => {});
    ...
    return {
      ...
      // nmemonic, opcode pairs
      opIndex: Object.fromEntries(
